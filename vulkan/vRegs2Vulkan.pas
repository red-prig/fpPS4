...
var
 scr:TVkExtent2D;
 RENDER_OVERRIDE  :TDB_RENDER_OVERRIDE;
...
begin
 ...
 RENDER_OVERRIDE  :=CX_REG^.DB_RENDER_OVERRIDE;
 ...
 if (DB_Z_INFO.FORMAT<>0) then
 if (DEPTH_CONTROL.Z_ENABLE<>0) then
 begin
  if (DB_DEPTH_VIEW.Z_READ_ONLY<>0) or (DEPTH_CONTROL.Z_WRITE_ENABLE=0) then
  begin
   //readonly
 ...
 if ((Result.DEPTH_USAGE and TM_WRITE)<>0) then
 begin
  Assert(RENDER_OVERRIDE.FORCE_Z_DIRTY      =0,'FORCE_Z_DIRTY');
  Assert(RENDER_OVERRIDE.FORCE_Z_VALID      =0,'FORCE_Z_VALID');
 end;

 if ((Result.STENCIL_USAGE and TM_WRITE)<>0) then
 begin
  Assert(RENDER_OVERRIDE.FORCE_STENCIL_DIRTY=0,'FORCE_STENCIL_DIRTY');
  Assert(RENDER_OVERRIDE.FORCE_STENCIL_VALID=0,'FORCE_STENCIL_VALID');
 end;
 ...
 if (DB_Z_INFO.FORMAT<>0) then
 if (DEPTH_CONTROL.Z_ENABLE<>0) then
 if (DEPTH_CONTROL.Z_WRITE_ENABLE<>0) then
 ...
 Assert(RENDER_OVERRIDE.DISABLE_VIEWPORT_CLAMP=0,'DISABLE_VIEWPORT_CLAMP');
 ...
 Case PV^.nfmt of
  ...
  BUF_NUM_FORMAT_SNORM:
   case PV^.dfmt of
    ...
    BUF_DATA_FORMAT_2_10_10_10 :Result:=VK_FORMAT_A2R10G10B10_SNORM_PACK32;
    else;
   end;
